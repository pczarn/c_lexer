...
pub fn parse_number_radix(
    ...
    let i = u64::from_str_radix(i, u32::from(radix))?;
    *c_src -= 1;
...
pub fn parse_number(input: &[u8], c_src: &mut usize, token_len: u64) -> Result<Token, Error> {
    ...
    let i = u64::from_str_radix(i, 10).unwrap_or(0);
    *c_src -= 1;
...
pub fn parse_number_decimal(
    ...
    let integer = u64::from_str_radix(integer, 10).unwrap_or(0);

    let decimal = unsafe { str::from_utf8_unchecked(&input[i_point + 1..*c_src - 1]) };
    let decimal = u64::from_str_radix(decimal, 10).unwrap_or(0);
